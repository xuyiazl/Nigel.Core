namespace Nigel.Core
{
    ...
    *           Created by 徐毅 at 2011/11/29 12:56:20
    ...
    ********************************************************************/
    ...
    {
        public static System.Linq.Expressions.Expression<T> LinqOr<T>(this System.Linq.Expressions.Expression<T> source,
              System.Linq.Expressions.Expression<T> dest)
        {
            var changedExp = new CusExpressVisitor(source.Body, source.Parameters[0], dest.Parameters[0]).Visit(source.Body);
            return System.Linq.Expressions.LambdaExpression.Lambda<T>(
                System.Linq.Expressions.LambdaExpression.Or(changedExp, dest.Body), dest.Parameters);
        }

        public static System.Linq.Expressions.Expression<T> LinqAnd<T>(this System.Linq.Expressions.Expression<T> source,
                   System.Linq.Expressions.Expression<T> dest)
        {
            var changedExp = new CusExpressVisitor(source.Body, source.Parameters[0], dest.Parameters[0]).Visit(source.Body);
            return System.Linq.Expressions.LambdaExpression.Lambda<T>(
                System.Linq.Expressions.LambdaExpression.AndAlso(changedExp, dest.Body), dest.Parameters);
        }
    ...
    /// 统一linq表达式的参数
    /// </summary>
    class CusExpressVisitor : System.Linq.Expressions.ExpressionVisitor
    {
        private System.Linq.Expressions.ParameterExpression _sourceParas, _destParas;
        private System.Linq.Expressions.Expression _source;

        public CusExpressVisitor(System.Linq.Expressions.Expression Source,
            System.Linq.Expressions.ParameterExpression SourceParas,
            System.Linq.Expressions.ParameterExpression DestParas)
        {
            this._sourceParas = SourceParas;
            this._destParas = DestParas;
        }
